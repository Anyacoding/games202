...
bool RayMarch(vec3 ori, vec3 dir, out vec3 hitPos) {
  float step = 0.05;
  const int maxStepTimes = 150;

  vec3 stepDir = normalize(dir) * step;
  vec3 curPos = ori;

  for (int i = 0; i < maxStepTimes; ++i) {
...
void main() {
  ...
  // L = (GetGBufferDiffuse(uv) + EvalReflect(wi, wo, uv)) / 2.0;
    ...
    vec3 dir = normalize(mat3(b1, b2, normal) * localDir);
		...
		if (RayMarch(posWorld, dir, hitPos)) {
			vec2 hitUV = GetScreenCoordinate(hitPos);
			L_indirect += EvalDiffuse(dir, wo, uv) * EvalDiffuse(wi, dir, hitUV) * EvalDirectionalLight(hitUV) / pdf;
		}
	}
